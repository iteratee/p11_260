...
int main(int _argc, char **argv) {
  residue_narrow_t x = {
    .limbs = {
      0x3553e74, 0x0464e4c, 0x61de408, 0x006a30e,
      0x6e9b25b, 0x3e6f39e, 0x19ec754, 0x5c71cc3,
      0x2bc1c0e, 0x554338e, 0x14e8b6e,
    },
  };

  residue_narrow_t two = {
    .limbs = {0x2},
  };

  residue_narrow_t x_plus_two;
  ...
  residue_narrow_t y = {
    .limbs = {
      0x5f5b0e1, 0x4668277, 0x0f7d85a, 0x4515e42,
      0x00cb559, 0x3f8a910, 0x6655708, 0x3085b4d,
      0x581ceff, 0x3324c03, 0x56ed38e,
    },
  };
  ...
  residue_wide_t mul_expected = {
    .limbs = {
      0x1c508c4, 0x3eeb85d, 0x04bc914, 0x0a57e1c,
      0x1f13f9a, 0x2d8aa7d, 0x232cce3, 0x31e92c4,
      0x04fb073, 0x2582507, 0x06e9e1d,
    },
  };

  residue_wide_t square_expected = {
    .limbs = {
      0x2073353, 0x18e5de4, 0x320a4ab, 0x3ee123a,
      0x2d88419, 0x3d1ae13, 0x02b3dcf, 0x2997027,
      0x3d550a2, 0x220a052, 0x3088d3c,
    },
  };
  ...
  residue_narrow_t negative_one_redundant = {
    .limbs = {
      ...
      0x3ffffff, 0x3ffffff, 0x3ffffff, 0x3ffffff,
      0x3ffffff, 0x3ffffff, 0x000000e,
    },
  };

  residue_narrow_t negative_t2_plus_one = {
    .limbs = {
      0x3ffffff, 0x000000e, 0x3ffffff, 0x3ffffff,
      0x3ffffff, 0x3ffffff, 0x3ffffff, 0x3ffffff,
      0x3ffffff, 0x3ffffff, 0x000000e,
    },
  };
  ...
  residue_narrow_t sqrt_x_plus_2_over_y = {
    .limbs = {
      0x2c7e63f, 0x33dcf44, 0x2809ebf, 0x20d7809,
      0x272f0aa, 0x3444d37, 0x201dbc6, 0x3a02a7,
      0x13c3650, 0x32d94d8, 0x30e1c97,
    },
  };

  residue_narrow_t x_inverse = {
    .limbs = {
      0x17a9f53, 0x22e2983, 0x0f09456, 0x11fb41e,
      0x1e47b3f, 0x37dd25f, 0x3bc6938, 0x2b654cd,
      0x233a0b2, 0x3f8c25b, 0x09fd09b,
    },
  };
  ...
  // one any more.
  reduce_step_narrow(&result, &negative_one_redundant);
  for (int i = 0; i < NLIMBS; ++i) {
    assert(negative_one_redundant.limbs[i] == result.limbs[i]);
  }

  reduce_step_narrow(&result, &negative_t2_plus_one);
  for (int i = 0; i < NLIMBS; ++i) {
    assert(negative_t2_plus_one.limbs[i] == result.limbs[i]);
  }
  ...
  assert(!sqrt_inv_narrow(&result, &x, &y));
  add_narrow(&x_plus_two, &x, &two);
  assert(sqrt_inv_narrow(&result, &x_plus_two, &y));
  for (int i = 0; i < NLIMBS; ++i) {
    assert(sqrt_x_plus_2_over_y.limbs[i] == result.limbs[i]);
  }
  {
    residue_narrow_t tmp, tmp2;
    square_narrow(&tmp, &result);
    mul_narrow(&tmp2, &tmp, &y);
    assert(equal_narrow(&tmp2, &x_plus_two));
  }

  invert_narrow(&result, &x);
  assert(equal_narrow(&result, &x_inverse));
  ...
  projective_pt_narrow_t expected_scalar_mult = {
    .x = {
      .limbs = {
        0x1267d8d, 0x39a3cd3, 0x09e1275, 0x2d21378, 0x24771d9, 0x3558a1d,
        0x3bdca9b, 0x0dd862d, 0x0bb230a, 0x1668292, 0x0350abe,
      },
    },
    .y = {
      .limbs = {
        0x04d69fd, 0x03e739d, 0x36ce258, 0x0b6464b, 0x19dab22, 0x249c1a8,
        0x1d28c7d, 0x1591dbc, 0x085ebab, 0x0e8274f, 0x0b090d6,
      },
    },
    .z = {
      .limbs = {0x1},
  ...
  {
    residue_narrow_t tmp;
    mul_narrow(&tmp, &expected_scalar_mult.x, &result_pt.z);
    assert(equal_narrow(&tmp, &result_pt.x));
    mul_narrow(&tmp, &expected_scalar_mult.y, &result_pt.z);
    assert(equal_narrow(&tmp, &result_pt.y));
  }

  affine_pt_narrow_t expected_everything0 = {
    .x = {
      .limbs = {
        0x20eef1a, 0x3c30e66, 0x0d710f0, 0x248a6fa, 0x30c967f, 0x3ce302c,
        0x0ccd1f2, 0x197e993, 0x2ebaef3, 0x0f2f019, 0,
      },
    },
    .y = {
      .limbs = {
        0x3017cc0, 0x02a5110, 0x06d37e5, 0x283a64a, 0x01484b5, 0x196f37b,
        0x13de2d2, 0x0da32d1, 0x392e0fc, 0x221d742, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_everything1 = {
    .x = {
      .limbs = {
        0x0e35d45, 0x038f90c, 0x0283483, 0x01ee50a, 0x1e364f9, 0x362414c,
        0x156b1ed, 0x006fff6, 0x271f9ed, 0x0ffa45d, 0,
      },
    },
    .y = {
      .limbs = {
        0x156ae67, 0x27941ab, 0x19a3000, 0x3572ab5, 0x2b90ce3, 0x136156c,
        0x0727496, 0x0edae82, 0x0fa5dfd, 0x16f293c, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_everything2 = {
    .x = {
      .limbs = {
        0x37fcb1b, 0x16004b9, 0x1d18743, 0x0bce648, 0x0d78db6, 0x35b1d65,
        0x23bb620, 0x2fbc323, 0x1a9a586, 0x3b22577, 0,
      },
    },
    .y = {
      .limbs = {
        0x082fb15, 0x03487d6, 0x3d1c2c9, 0x2c9e7ad, 0x187be10, 0x2e9b6ba,
        0x15b8f89, 0x243ae4c, 0x328bb11, 0x00b12a9, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_everything3 = {
    .x = {
      .limbs = {
        0x3e79b25, 0x2ca71b7, 0x2b2ea3c, 0x0de7ac4, 0x3026d10, 0x2bce79e,
        0x1153866, 0x03e5a80, 0x22b9a37, 0x03e9c59, 0,
      },
    },
    .y = {
      .limbs = {
        0x20100d6, 0x2330974, 0x3402585, 0x172cfd6, 0x275a21c, 0x213e87c,
        0x29989f2, 0x155e437, 0x096a378, 0x3a674eb, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_gray_code_end0 = {
    .x = {
      .limbs = {
        ...
        0x2562a88, 0x0be34f0, 0x192ead9, 0x089ec45, 0,
      },
    },
    .y = {
      .limbs = {
        0x1de5221, 0x172f820, 0x28c1b33, 0x08003c6, 0x0e65926, 0x188cd49,
        0x3bb39fd, 0x1b9d8d7, 0x03d5020, 0x045742b, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_gray_code_end1 = {
    .x = {
      .limbs = {
        ...
        0x2737d9c, 0x2ca9eba, 0x058f290, 0x3879a7c, 0,
      },
    },
    .y = {
      .limbs = {
        0x357399d, 0x0276752, 0x0d5199f, 0x1bbd3a0, 0x39044f1, 0x0c5e83a,
        0x1a99cdd, 0x0dcb61f, 0x35b7272, 0x1184cff, 0,
      },
    },
  };
  ...
  affine_pt_narrow_t expected_gray_code_end2 = {
    .x = {
      .limbs = {
        0x1ea3c19, 0x081dc9e, 0x1a0b337, 0x1d7f3f4, 0x295a0aa, 0x1ebff45,
        0x0956bf0, 0x17aae80, 0x05d8632, 0x3082c9a, 0,
      },
    },
    .y = {
      .limbs = {
  ...
  affine_pt_narrow_t expected_gray_code_end3 = {
    .x = {
      .limbs = {
        0x06b9c9d, 0x3d00674, 0x10a73fc, 0x30fda83, 0x139185c, 0x043e082,
        0x3c67915, 0x208192a, 0x025e451, 0x258a566, 0,
      },
    },
    .y = {
      .limbs = {
        0x3d2a04f, 0x1314c36, 0x131c7a3, 0x1882ef3, 0x1a0a5e8, 0x1919356,
        0x0a5616a, 0x1eea31d, 0x2c216b3, 0x18ba4aa, 0,
      },
    },
  };
  ...
  assert(equal_narrow(&computed_base_comb.combs[0].table[COMB_TABLE_SIZE - 1].x,
    &expected_everything0.x));
  assert(equal_narrow(&computed_base_comb.combs[0].table[COMB_TABLE_SIZE - 1].y,
    &expected_everything0.y));
  assert(equal_narrow(&computed_base_comb.combs[1].table[COMB_TABLE_SIZE - 1].x,
    &expected_everything1.x));
  assert(equal_narrow(&computed_base_comb.combs[1].table[COMB_TABLE_SIZE - 1].y,
    &expected_everything1.y));
  assert(equal_narrow(&computed_base_comb.combs[2].table[COMB_TABLE_SIZE - 1].x,
    &expected_everything2.x));
  assert(equal_narrow(&computed_base_comb.combs[2].table[COMB_TABLE_SIZE - 1].y,
    &expected_everything2.y));
  assert(equal_narrow(&computed_base_comb.combs[3].table[COMB_TABLE_SIZE - 1].x,
    &expected_everything3.x));
  assert(equal_narrow(&computed_base_comb.combs[3].table[COMB_TABLE_SIZE - 1].y,
    &expected_everything3.y));


  assert(equal_narrow(&computed_base_comb.combs[0].table[7].x,
    &expected_gray_code_end0.x));
  assert(equal_narrow(&computed_base_comb.combs[0].table[7].y,
    &expected_gray_code_end0.y));
  assert(equal_narrow(&computed_base_comb.combs[1].table[7].x,
    &expected_gray_code_end1.x));
  assert(equal_narrow(&computed_base_comb.combs[1].table[7].y,
    &expected_gray_code_end1.y));
  assert(equal_narrow(&computed_base_comb.combs[2].table[7].x,
    &expected_gray_code_end2.x));
  assert(equal_narrow(&computed_base_comb.combs[2].table[7].y,
    &expected_gray_code_end2.y));
  assert(equal_narrow(&computed_base_comb.combs[3].table[7].x,
    &expected_gray_code_end3.x));
  assert(equal_narrow(&computed_base_comb.combs[3].table[7].y,
